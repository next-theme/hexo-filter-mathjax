...
function useSynchronousAsyncLoad() {
  mathjax.asyncLoad = name => requirePath(name.startsWith('@') ? name : Package.resolvePath(name));
...
function filter(config) {
  ...
  async function render(content) {
    ...
    //
    //  Create input jax and a document using them on the content from the HTML file
    //
    await ready;
    ...
    //
    //  Reset the shared output jax before typesetting a new document
    //
    svg.reset();
    svg.svgStyles = null;
  ...
  return function(content) {
    const output = renderQueue.then(() => render(content));
    renderQueue = output.catch(() => {});
    return output;
  };
}
